...
Migration **0098** — `supabase/migrations/20260802120000_0142_ciclo_de_vida_do_contexto.sql`.

```sql
-- ---- marca de corte do contexto do agente (migration 0142) ----
...
-- ---- política de expiração por etapa do Kanban (migration 0142) ----
...
1. `supabase/migrations/20260802120000_0142_ciclo_de_vida_do_contexto.sql` — DDL de §3, idempotente (`add column if not exists`, `create index if not exists`, constraint guardada por `do $$`).
2. Apêndice em `supabase/baseline.sql`, rotulado `-- ---- ciclo de vida do contexto (migration 0142) ----`, idempotente e auto-curativo — é o que o `install.sh` e o `update.sh` do kit self-host aplicam.
...
C1 entrega valor sozinha e sem risco de schema. C2 e C3 compartilham a migration 0142 — se forem entregues em PRs separados, a migration sai inteira em C2 e C3 apenas passa a usar as colunas.
