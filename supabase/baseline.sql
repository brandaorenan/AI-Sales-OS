...
-- ---- ciclo de vida do contexto do agente (migration 0218) ----
...
-- ---- hard reset atômico do contexto (migration 0219) ----
...
-- ---- debounce inbound por organização (migration 0220) ----
...
-- ---- hard reset não é de quem está logado (migration 0221) ----
-- `fn_hard_reset_contact_context` (0219) é DEFINER e ESCREVE, e recebe a
...
-- tenant. O `revoke ... from public` da 0219 não cobre isso — são duas origens
