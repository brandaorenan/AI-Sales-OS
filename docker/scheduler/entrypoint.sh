...
# event-log-drain TAMBÉM roda aqui fora do crontab, a cada 3s: este drain está
# no CAMINHO CRÍTICO da resposta ao cliente — é ele que move
# media.persist_requested → media.derive_requested (áudio virando texto antes
# de o agente responder). A resolução de 1 min do crond deixava dois hops de
# até um minuto cada (~75s de transcrição) e, de quebra, estourava a validade
# da URL de mídia assinada pelo provedor do canal (download voltando 404
# quando o segundo hop finalmente chegava). A linha de crontab acima continua
# existindo como rede de segurança.
#
# `$$` continua apontando para ESTE processo mesmo depois do `exec crond` logo
# abaixo (exec troca o programa, não o PID) — então o loop nasce filho do
# crond e para sozinho quando ele morre. Sem essa checagem, o teste de shell
# que invoca este script fora de um contêiner (sem cgroup para podar órfão)
# vazaria um loop rodando para sempre a cada execução.
ENTRYPOINT_PID=$$
(
  while kill -0 "$ENTRYPOINT_PID" 2>/dev/null; do
    curl -fsS -m45 -H "Authorization: Bearer $SEGREDO_SEGURO" "$APP_ORIGIN/api/v1/cron/event-log-drain" >/dev/null 2>&1 || true
    sleep 3
  done
) &
