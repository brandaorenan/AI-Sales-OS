...
setup_event_log_drain_cron() {
  ...
  # O lock leva o diretório da instalação no nome pela MESMA razão do marcador:
  # duas instalações na mesma VPS disputariam um lock global e só uma das duas
  # teria drain — a outra ficaria muda, sem erro nenhum aparecendo.
  local lock_file; lock_file="/tmp/deskcomm-drain$(printf '%s' "${PROJECT_DIR:-$PWD}" | tr -c 'a-zA-Z0-9' '_').lock"
  local cron_line="* * * * * flock -n ${lock_file} -c 'while true; do curl -fsS -m45 -H \"Authorization: Bearer ${secret}\" \"${url_drain}\" >/dev/null 2>&1 || true; sleep 3; done' ${marcador}"
