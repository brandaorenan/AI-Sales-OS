...
const COLUNAS =
  "id, name, slug, position, is_won, is_lost, is_archived, agent_stage_hint, resets_context, context_reset_after_days, last_change_actor_kind, last_change_at";
...
export interface EtapaVisivel {
   ...
   * Viaja na MESMA projeção que a autoria porque a tela de etapas desenha as
   * duas juntas — sem elas aqui, o checkbox e a carência nasceriam sempre
   * desmarcados, ignorando o que está salvo.
   */
...
export interface PedidoDeEdicao {
  ...
  /**
   * Política de expiração do contexto do agente (Spec 16 §9.1). A rota já cobra
   * `admin` para estes dois campos — aqui é só transporte da decisão, como o
   * resto do pedido.
   */
  resets_context?: boolean;
  context_reset_after_days?: number;
...
/** Só estes dois campos são a política de contexto — o resto do pedido não é. */
function tocaPoliticaDeContexto(pedido: PedidoDeEdicao): boolean {
...
export async function atualizarEtapa(
  ...
  // `pipeline.stage_updated` cobre a EDIÇÃO da etapa (nome, marcação, ordem).
  // Mudar só a política de contexto não é editar a etapa: tem audit própria
  // logo abaixo, e emitir as duas faria a mesma mudança contar duas vezes em
  // relatórios que somam ações de configuração.
  const soPolitica =
    tocaPoliticaDeContexto(pedido) &&
    pedido.name === undefined &&
    pedido.depois_de === undefined &&
    !temMarcacao;
  if (updates.length > 0 && !soPolitica) {
    const autor = autorDoAudit(deps.actor);
    void audit({
  ...
  // A política de contexto tem audit PRÓPRIA, e não é redundância com a de
  // cima: mudar quando o agente esquece a conversa é decisão de retenção de
  // dado (Spec 16 §9.1), lida por quem audita LGPD — não pode ficar diluída
  // dentro do `pedido` de um `pipeline.stage_updated` genérico.
