...
export async function runModelCall(db: pg.Pool, cfg: LlmEdgeConfig, input: RunModelCallInput, deps: RunModelCallDeps = {}) {
  ...
  try {
    ...
    const esforco = esforcoParaChamada(config.provider, model, purpose);
...
export function redigirMensagemDoProvedor(bruto: string): string {
  const semSegredo = bruto
    ...
    // `\b` é word-boundary (não backspace). Sem o escape, a âncora vira
    // literal `\x08` e `Authorization: Bearer sk-…` passa intacto pra tela.
    ...
    .replace(/\b(x-api-key|api[-_]?key|authorization)\s*[:=]\s*\S+/gi, '$1: [CHAVE]');
