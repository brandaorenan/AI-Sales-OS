...
import { latestCheckpoint, loadContextResetAt } from './inbound-turn';
 ...
 *
 * ⚠️ A MARCA DE CORTE VALE AQUI TAMBÉM (Spec 16 §5.2). Ler o checkpoint sem
 * filtrar por `contacts.context_reset_at` faria o Operador agir sobre o que o
 * Conversador foi proibido de lembrar — o cliente pede "esquece tudo", a
 * conversa recomeça, e o Operador continua cumprindo promessas do ciclo
 * anterior. O corte só FILTRA leitura; nada é apagado.
 */
export async function lerDeclaracaoDoTurno(
  ...
  const checkpoint = await latestCheckpoint(db, tenantId, leadId, contextResetAt);
