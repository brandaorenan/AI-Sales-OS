...
import { checkpointDoJob, loadContextResetAt } from './inbound-turn';
...
/**
 * Lê a declaração do checkpoint DO TURNO que originou este Operador.
 *
 ...
 * é anterior a um corte de contexto) ou o checkpoint existe e não trouxe
 * declaração (o modelo não declarou). Os dois levam o Operador a RODAR, não a
 * pular — nos dois casos ninguém avaliou o turno, que é a condição em que ele
 * mais importa.
 *
 ...
 *
 * ⚠️ A MARCA DE CORTE VALE AQUI TAMBÉM (Spec 16 §5.2). Um checkpoint anterior a
 * `contacts.context_reset_at` é tratado como inexistente — sem isso o Operador
 * agiria sobre o que o Conversador foi proibido de lembrar: o cliente pede
 * "esquece tudo", a conversa recomeça, e o Operador continua cumprindo
 * promessas do ciclo anterior. O corte só FILTRA leitura; nada é apagado.
 */
export async function lerDeclaracaoDoTurno(
  ...
  const contextResetAt = await loadContextResetAt(db, tenantId, leadId);
  if (contextResetAt !== null && checkpoint.created_at.getTime() <= new Date(contextResetAt).getTime()) {
    return { declaracao: null, houveCheckpoint: false };
  }
