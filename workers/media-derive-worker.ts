...
function buildDeriveDeps(
  ...
  // Sem chave OpenAI não há como transcrever — e o caminho de erro grava o
  // marcador em vez de string vazia (ver abaixo), o que evita tanto o loop de
  // 401 a cada drain quanto o agente responder como se o áudio não existisse.
  // `TRANSCRIPTION_MODEL`/`TRANSCRIPTION_BASE_URL` são opcionais: ausentes, o
  // provider usa o default — é o estado de um primeiro deploy.
